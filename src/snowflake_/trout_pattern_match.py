...
def _clean_data(df: pd.DataFrame, cols = [str]) -> None:
    ...
    for col in cols:
        ...
        df[clean_col_name] = df[clean_col_name].str.replace("(lake|reservoir)", "", regex=True)

        df[clean_col_name] = df[clean_col_name].str.replace("[^a0-zA9-Z ]", "", regex=True)
...
def _pattern_match_data(master_angler_df: pd.DataFrame, species_df: pd.DataFrame) -> pd.DataFrame:
    ...
    compare.string("location", "water", method='damerau_levenshtein', label="levenshtein_comparison")
...
def _select_best_match(compare_vectors: pd.DataFrame) -> pd.DataFrame:
    ...
    comparison_df["total_score"] = comparison_df.jaro_comparison + comparison_df.levenshtein_comparison

    matched_comparison_df = comparison_df.loc[comparison_df.total_score >= 1.2]

    best_mtach_index = matched_comparison_df.groupby(by="master_angler_award_id").total_score.idxmax().to_list()
...
def match_fishing_data(session: Session) -> str:
    ...
    species_df = session.sql("select * from STORAGE_DATABASE.CPW_DATA.ALL_SPECIES").to_pandas()
